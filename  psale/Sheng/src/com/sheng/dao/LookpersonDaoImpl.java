...
public class LookpersonDaoImpl implements LookpersonDAO {
	 ...
	 * 查看指定人员，指定日期内的销量 *
	 */
	public List<Outwuliao> getpersonout(String userid, Timestamp startdate,
			Timestamp enddate) {
		...
		try {
			...
			pm = conn
					.prepareStatement("select * from delwuliao where outuserid=? and outdate between ? and ?");
	...
	public List<Outwuliao> getpersonoutbypage(String userid,
		...
		try {
			conn = Pool.getConnection();
			pm = conn
					.prepareStatement("select * from delwuliao where outuserid=? and outdate between ? and ? limit "
	...
	public List<Double> getsumms(String userid, Timestamp startdate,
		...
		try {
			conn = Pool.getConnection();
			pm = conn
					.prepareStatement("select sum(outprice),sum(maori) from delwuliao where outuserid=? and outdate between ? and ?");
			pm.setString(1, userid);
			pm.setTimestamp(2, startdate);
			pm.setTimestamp(3, enddate);
			rs = pm.executeQuery();
	...
	public List<Integer> getsumpage(String userid, Timestamp startdate,
		...
		try {
			conn = Pool.getConnection();
			pm = conn
					.prepareStatement("select count(*) from delwuliao where outuserid=? and outdate between ? and ?");
			pm.setString(1, userid);
			pm.setTimestamp(2, startdate);
			pm.setTimestamp(3, enddate);
			rs = pm.executeQuery();
