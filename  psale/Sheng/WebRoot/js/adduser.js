...
function checkusername(t){
	...
	if(username.length==0){
		var tt=l.top-35+document.body.scrollLeft;
		var ll=l.left+130+document.body.scrollTop;
		$("#usernameerror")[0].style.left=ll+"px";
		$("#usernameerror")[0].style.top=tt+"px";
		$("#usernameerror").show();
...
function checkuserexit(t){
	...
	var l=$(t).offset();
	if(checkuserid()){
		$.getJSON("Checkuseraction.action",{userid:userid},function(date){
			if(date.showmessage==1){
				$("#useridcontent").html("<font color='red'>这个ID已经被使用过了，请再选一个</font>");
				var tt=l.top-35+document.body.scrollLeft;
				var ll=l.left+130+document.body.scrollTop;
